#!/usr/bin/env python3
"""Test with reduced trading frequency (5% threshold)"""
...
print("="*60)
print("BACKTEST WITH FEE REDUCTION FIXES")
print("="*60)
print("Changes: 5% rebalancing threshold (was 1%), fee tracking")
print("="*60)
...
total_trade_value = sum(t['value'] for t in test_env.trades)
total_fees = test_env.total_fees_paid
fee_percentage = (total_fees / initial) * 100
...
print(f"  Trade Frequency:     Every {len(test_data)/len(test_env.trades) if test_env.trades else 0:.1f} hours")

print("\n💰 FEE ANALYSIS:")
print(f"  Total Trade Value:   ${total_trade_value:,.2f}")
print(f"  Total Fees Paid:     ${total_fees:.2f}")
print(f"  Fees % of Capital:   {fee_percentage:.2f}%")
print(f"  Avg Fee per Trade:   ${total_fees/len(test_env.trades) if test_env.trades else 0:.2f}")
...
print("COMPARISON")
print("="*60)
print("Before (1% threshold): -0.33% alpha, 211 trades")
print(f"After (5% threshold):  {returns - buy_hold:+.2f}% alpha, {len(test_env.trades)} trades")
print(f"Fee reduction:         From ~10% to {fee_percentage:.2f}%")
print("="*60)